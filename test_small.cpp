...
int main(int argc, char** argv)
{
	...
	load_image(image, "chiungi1_s.jpg");
	images.push_back(std::move(image));
	load_image(image, "chiungi2_s.jpg");
	images.push_back(std::move(image));
	load_image(image, "chiungi3_s.jpg");
	images.push_back(std::move(image));
	load_image(image, "tzuting1_s.jpg");
	images.push_back(std::move(image));
	load_image(image, "tzuting2_s.jpg");
	images.push_back(std::move(image));
	load_image(image, "tzuting3_s.jpg");
	images.push_back(std::move(image));
	
	std::vector<matrix<float,0,1>> features = net(images);
	
	ofstream out("features_small.bin",std::ios_base::binary);
	if(!out.good())
	{
		cout << "Error" << endl;
	}
    ...
    for (auto face : detector(image))
    {
        ...
        extract_image_chip(image, get_face_chip_details(shape,32,0.25), face_chip);
