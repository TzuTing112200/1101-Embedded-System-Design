...
int main()
{
    try
    {
        ...
        while(cap.read(frame))
        {
			...
			cout << chrono::duration<double, milli>(t_end-t_start).count() / embedded.size() << " ms" << endl;
			...
			for  (size_t i = 0; i < embedded.size(); ++i)
			{
				...
				if (length(embedded[i]-features[0]) < net.loss_details().get_distance_threshold())
					...
					label = "chiungi";
				else if (length(embedded[i]-features[2]) < net.loss_details().get_distance_threshold())
					label = "chiungi";
				else if (length(embedded[i]-features[3]) < net.loss_details().get_distance_threshold())
					label = "tzuting";
				else if (length(embedded[i]-features[4]) < net.loss_details().get_distance_threshold())
					label = "tzuting";
				else if (length(embedded[i]-features[5]) < net.loss_details().get_distance_threshold())
					label = "tzuting";
				else
					label = "unknown" ;
