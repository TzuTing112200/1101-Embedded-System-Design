...
int main(int argc, char** argv)
{
    ...
    trainer.be_verbose();
    trainer.set_synchronization_file("face_metric_sync_small", std::chrono::minutes(5));
    ...
    auto data_loader = [&qimages, &qlabels, &objs](time_t seed)
    {
        ...
        while(qimages.is_enabled())
        {
            try
            {
                load_mini_batch(2, 8, rnd, objs, images, labels);
	...
	int index = 0;
    while(trainer.get_learning_rate() >= 1e-4 && ++index < 3000)
    {
        cout << index << endl;
    ...
    net.clean();
    serialize("metric_network_renset_small.dat") << net;
