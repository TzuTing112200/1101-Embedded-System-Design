...
int main(int argc, char** argv)
{
	...
	load_image(image, "chiungi1.jpg");
	images.push_back(std::move(image));
	load_image(image, "chiungi2.jpg");
	images.push_back(std::move(image));
	load_image(image, "chiungi3.jpg");
	images.push_back(std::move(image));
	load_image(image, "tzuting1.jpg");
	images.push_back(std::move(image));
	load_image(image, "tzuting2.jpg");
	images.push_back(std::move(image));
	load_image(image, "tzuting3.jpg");
	images.push_back(std::move(image));
	
	std::vector<matrix<float,0,1>> features = net(images);
	
	ofstream out("features.bin",std::ios_base::binary);
	if(!out.good())
	{
		cout << "Error" << endl;
	}
