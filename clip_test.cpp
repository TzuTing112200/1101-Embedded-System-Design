...
int main(int argc, char** argv) try
{
    ...
    //anet_type net;
    //deserialize("/home/es/workspace/Source/dlib_face_recognition_resnet_model_v1.dat") >> net;
    ...
    for (auto face : detector(img))
    {
        ...
        extract_image_chip(img, get_face_chip_details(shape,50,0.25), face_chip);
        faces.push_back(move(face_chip));
        // Also put some boxes on the faces so we can see that the detector is finding
        // them.
        //win.add_overlay(face);
		string filename = string(argv[1]);
		save_jpeg(faces[0], "images_50/" + filename.substr(filename.rfind("/"), filename.rfind(".") - filename.rfind("/")) + ".jpg");
    }
	/*
    if (faces.size() == 0)
    {
